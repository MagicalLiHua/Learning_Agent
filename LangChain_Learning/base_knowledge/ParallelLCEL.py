...
class RunnableParallel(Runnable):
    def __init__(self, steps_dict):
        # steps_dict 是一个字典，例如 {"humor": model1, "serious": model2}
        self.steps_dict = steps_dict

    async def ainvoke(self, input_data):
        print(f"🔀 开启并行模式，正在同时处理 {list(self.steps_dict.keys())}...")

        # 核心逻辑：利用 asyncio.gather 并行执行所有任务
        keys = list(self.steps_dict.keys())
        tasks = [runnable.ainvoke(input_data) for runnable in self.steps_dict.values()]
...
async def main():
    ...
    # 1. 准备两套不同的提示词
    humor_prompt = PromptTemplate("用幽默的话解释【{topic}】。")
    serious_prompt = PromptTemplate("用专业严谨的话解释【{topic}】。")
    model = QwenLLM(SERVER_URL, MODEL)

    # 2. 构建并行分支
    # 这里定义了两个分支：'humor_version' 和 'serious_version'
    parallel_branch = RunnableParallel({
        "humor_version": humor_prompt | model,
        "serious_version": serious_prompt | model
    })

    # 3. 运行链条
    print("--- 🚀 开始运行并行链条 ---")
    final_output = await parallel_branch.ainvoke({"topic": "Python 异步编程"})

    print(f"\n🎭 【幽默版】: {final_output['humor_version']}")
    print(f"🎓 【严谨版】: {final_output['serious_version']}")
